'1. Cargar un matriz de M x N elementos con números y realizar (utilizando para todos los casos
'procedimientos)
'a.Generar un vector con la sumatoria de cada fila.
'b.Generar un vector con la sumatoria de cada columna.
'c.Generar un vector con las posiciones de los mayores de cada fila.
'd.Generar un vector con las posiciones de los menores de cada columna.
'e.Generar un vector con el promedio de cada fila.
'f.Generar un vector con el promedio de cada columna.
'g.Generar un vector con los elementos de la fila k (siendo está ingresada al comenzar).
'h.Generar un vector con todos los elementos que superen en valor absoluto al promedio general
'de la matriz.
Module Module1
    Sub Main()
        Dim mat(,), m, n, k As Integer
        'ingreso los valores de la fila y columna utilizando para ello la Función ingresarnumero
        ...
        Dim VecSumaFila(m), VecSumaColumna(n), VecMayorFila(m), VecMenColumna(n), VectorFilaK(n),
VectorValorAb() As Integer
        ...
        ' muestro matriz
        Console.WriteLine()
        Console.WriteLine("Valores cargados en la matriz")
        Console.WriteLine()
        mostrarmatriz(mat)
        Console.ReadLine()
        ' llamos al procedimiento que va a sumar a cada fila de la matriz pasando como parámetro la matriz y el
        ' vector
        ' donde se guardarán los resultados
        SumaFila(mat, VecSumaFila)
        ' llamo al procedimiento que me mostrará el vector
        MostraVector(VecSumaFila, "La suma", "Fila")
        ' llamo al procedimiento que va a sumar a cada columna de la matriz pasando como
        ...
        ' llamo al procedimiento que va a determinar la posición del menor de cada columna de la matriz pasando
        'como
        ' parámetro la matriz y el vector donde se guardarán los resultados
